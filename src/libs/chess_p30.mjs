import {PIECE_OFFSETS, ATTACKS, RAYS, PERMS} from './chesstables.mjs';

let ChessP30 = function (fen) {
  const BLACK = 'b';
  const WHITE = 'w';
  ...
  // Callbacks - may be defined by client code
  ...
  let game_result = {result: '*', reason: 'playing'};
  let turn = WHITE;
  ...
  function p30_fen_from_perm (perm) {
  ...
  function set_result (resultStr, reason) {
    game_result.result = resultStr;
    game_result.reason = reason;
    header.Result = resultStr;
    if (onResult) {
      onResult (game_result);
    }
  }

  function resigns (player) {
    if (player === 'white') {
      set_result ('0-1', 'White resigned');
    } else {
      set_result ('1-0', 'Black resigned');
    }
  }
  function lost_on_time (player) {
    if (player === 'white') {
      set_result ('0-1', 'White lost on time');
    } else {
      set_result ('1-0', 'Black lost on time');
    }
  }
  function draw_agreed () {
    set_result ('1/2', 'Draw agreed');
  }
  ...
  function generate_moves (options) {
    ...
    for (let i = first_sq; i <= last_sq; i++) {
      ...
      if (piece.type === PAWN) {
        ...
        /* Not a pawn */ for (
          let j = 0, len = PIECE_OFFSETS[piece.type].length;
          j < len;
          j++
        ) {
          ...
          while (true) {
            ...
            // Is this a dragon-bishop moving like a knight?
            const dbn_move =
              piece.type === BISHOP && PIECE_OFFSETS[KNIGHT].includes (offset);
            if (dbn_move && !legal_dragon_bishop_move (i, square)) {
              break;
            }
            ...
            /* break, if knight or king or DB moving like a knight */
            if (piece.type === 'n' || piece.type === 'k' || dbn_move) break;
          }
        }
      }
    }
  ...
  function in_checkmate () {
    if (in_check () && generate_moves ().length === 0) {
      set_result (turn === WHITE ? '0-1' : '1-0', 'Checkmate');
      return true;
    }
    return false;
  }
  ...
  function insufficient_material () {
    ...
    if (drawn) {
      set_result ('1/2', 'Insufficient material');
      return true;
    }
    return false;
  }
  ...
  function in_threefold_repetition () {
    ...
    if (repetition) {
      set_result ('1/2', 'Threefold repetition');
    }
    return repetition;
  }
  ...
  function make_move (move, check_for_game_over = false) {
    let us = turn;
    let them = swap_color (us);
    push (move);
    ...
    if (!(check_for_game_over && game_over ())) {
  ...
  function game_over () {
    return (
      game_result.result !== '*' || // result previously calculated
      fifty_moves () ||
      in_checkmate () ||
      in_stalemate () ||
  ...
  function fifty_moves () {
    if (half_moves >= 100) {
      set_result ('1/2', 'Fifty-move rule');
      return true;
    }
    return false;
  }
  ...
  return {
    ...
    resigns: function (player) {
      return resigns (player);
    },

    lost_on_time: function (player) {
      return lost_on_time (player);
    },

    draw_agreed: function () {
      return draw_agreed ();
    },
    ...
    pgn: function (options) {
      ...
      for (let i in header) {
         ...
         */
        pgn_result.push ('[' + i + ' "' + header[i] + '"]' + newline);
        header_exists = true;
      }

      if (header_exists && history.length) {
        pgn_result.push (newline);
      }

      let append_comment = function (move_string) {
        let comment = comments[generate_fen ()];
       ...
       */
      if (max_width === 0) {
        return pgn_result.join ('') + moves.join (' ');
      }

      let strip = function () {
        if (
          pgn_result.length > 0 &&
          pgn_result[pgn_result.length - 1] === ' '
        ) {
          pgn_result.pop ();
          return true;
        }
      ...
      for (let i = 0; i < moves.length; i++) {
        ...
        if (current_width + moves[i].length > max_width && i !== 0) {
          ...
          if (pgn_result[pgn_result.length - 1] === ' ') {
            pgn_result.pop ();
          }
        ...
        pgn_result.push (moves[i]);
        current_width += moves[i].length;
      }

      return pgn_result.join ('');
    },
    ...
    load_pgn: function (pgn, options) {
      ...
      let comment = decode_comment (moves[moves.length - 1]);
      if (comment !== undefined) {
        comments[generate_fen ()] = comment;
        moves.pop ();
      }
...
// console.log (t.fen_p30 ());
console.log (t.ascii ());
for (let m of ['f4', 'c5', 'g4', 'b6', 'Bg3', 'a6', 'f5', 'g5', 'fxg6']) {
...
t.load ('r1bqkbnr/ppppn2P/4P3/8/8/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1');
console.log (t.ascii ());
console.log (t.move ('hxg8=B'));
console.log (t.ascii ());
console.log (t.pgn ());

console.log (t.move ('Rb8'));
console.log (t.ascii ());
console.log (t.pgn ());
console.log (t.move ('Bf6#'));
console.log (t.ascii ());
console.log (t.pgn ());
console.log (t.game_over ());
